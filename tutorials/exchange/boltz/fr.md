---
name: Boltz
description: Echangez entre les différentes couches de Bitcoin tout en gardant le contrôle.
---
...
- **On Chain** : La chaîne principale de Bitcoin où les transactions sont confirmées toutes les 10 minutes en moyenne, les frais de transaction sont souvent élevés qui ne répond pas nécessairement aux besoins des utilisateurs.
...
- **Liquid Network** : une surcouche de Bitcoin créée par Blockstream permettant de faire des transactions rapides, confidentiels et aussi d'utiliser autres instruments financiers sur la base de Bitcoin.
...
Cela dit, l'importance de l'interopérabilité entre ces couches est principalement de fournir la flexibilité qu'il faut à un utilisateur pour exploiter tout ce que Bitcoin a offrir.

Boltz utilise les échanges atomiques qui sont des échanges directs, sans intermédiaires, entre différentes couches en utilisant des scripts Bitcoin pour garantir la sécurité et l'exécution simultanée des transactions.
...
Sélectionnez ensuite la couche à partir de laquelle vous souhaitez faire un échange atomique puis selectionnez la couche sur laquelle vous souhaitez recevoir les bitcoins.
...
Une fois vos configurations de base terminée, insérez le montant de votre échange atomique puis créez une facture Lightning du montant équivalant ou insérez plus simplement votre LNURL.
...
Ce fichier contient les 12 mots clés du portefeuille associée à vos échanges atomiques. Nous vous recommandons de suivre les directives que notre tutoriel sur la sauvegarde des mots de récupération afin de préserver durablement cette information capitale.
...
Une fois votre paiement effectué puis confirmée, vous recevez automatiquement l'équivalent sur votre portefeuille Lightning.
...
Vous êtes connaissez désormais Boltz, une plateforme d'échange unique en son genre qui effectue l'interopérabilité entre différentes couches du protocole Bitcoin. Nous vous recommandons de découvrir également Bitfinex, l'une des premières plateforme d'échange pair à pair de Bitcoin.
